...
from vessel_analysis import (analyze_vessel, PELL_BRIDGE_PIERS, BRIDGE_LAT, BRIDGE_LON,
                             predict_trajectory, assess_collision_risk, calculate_allision_probability)

# Page configuration
st.set_page_config(
    page_title="Pell Newport Bridge Vessel Monitor",
    page_icon="🚢",
    layout="wide"
)
...
with title_col:
    st.title("🌉 Pell Newport Bridge Vessel Collision Monitoring System")
...
with col1:
    ...
    # Create map centered on Pell Newport Bridge
    m = folium.Map(
    ...
    # Add Pell Newport Bridge marker (BLUE - infrastructure)
    folium.Marker(
        [BRIDGE_LAT, BRIDGE_LON],
        popup="Pell Newport Bridge",
        tooltip="Pell Newport Bridge",
        icon=folium.Icon(color='blue', icon='bridge', prefix='fa')
    ).add_to(m)

    # Add pier markers (BLUE - infrastructure)
    for pier_id, pier_data in PELL_BRIDGE_PIERS.items():
        folium.CircleMarker(
...
with col2:
    ...
    with st.expander("View Details", expanded=False):
        ...
        • **Location:** Narragansett Bay, Rhode Island (41.505°N, 71.349°W)
        • **Year Built:** 1969
        • **Owner:** Rhode Island Turnpike and Bridge Authority
        ...
        • **Total Bridge Length:** 11,248 feet (2.1 miles)
        • **Vertical Clearance:** 206 feet MHW
        • **Horizontal Clearance:** 1,600 feet (main span)
        • **Number of Piers:** 16
        """)
        ...
        • **Connects:** Jamestown (Conanicut Island) to Newport (Aquidneck Island)
        • **Toll Bridge:** Yes (eastbound only)
        """)
    ...
    with st.expander("View Details", expanded=False):
        ...
        st.markdown("""
        • **Chart Reference:** NOAA Chart 13223 (Narragansett Bay)
        • **Channel:** East Passage, Narragansett Bay
        • **Maintained Depth:** 35-50 feet (varies by location)
        • **Project Depth:** Varies
        """)

        st.markdown("### Clearances")
        st.markdown("""
        • **Vertical Clearance:** 206 feet MHW (Mean High Water)
        • **Horizontal Clearance:** 1,600 feet (main span)
        """)

        st.markdown("### Currents & Tides")
        st.markdown("""
        • **Maximum Current:** ~2 knots (ebb/flood)
        • **Tidal Range:** ~3-4 feet (Mean Range)
        • **Current Pattern:** Tidal flow through Narragansett Bay
        """)

        st.markdown("### Vessel Traffic")
        st.markdown("""
        • **Typical Vessels:** Naval vessels, ferries, fishing boats, recreational, cruise ships
        • **Naval Traffic:** Naval Station Newport nearby
        • **VTS Coverage:** Coast Guard Sector Southeastern New England
        """)

        st.markdown("### Pier-Specific Data")
        st.markdown("""
        • **Number of Piers:** 16 (Piers 1-16)
        • **Tower Piers:** Pier 6 and Pier 7
        • **Anchorage Piers:** Pier 3 and Pier 10
        • **Water Depth at Tower Piers:** ~50 feet
        • **Water Depth at Other Piers:** ~35 feet
        ...
        st.markdown("""
        *📊 Data sources:*
        - *Navigation: NOAA Nautical Chart 13223*
        - *Pier capacity: Placeholder - requires structural engineering analysis*
        - *Pier coordinates: Provided by user (GPS coordinates)*
...
**About Roebling Labs:** Vessel allision detection system for bridges using AIS data and computer vision.
Addresses NTSB Safety Recommendations from Francis Scott Key Bridge collapse investigation.
""")
